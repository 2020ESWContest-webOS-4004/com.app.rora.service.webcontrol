...
function address_search_displayresult(text, resultCoords) {
    var coords = new kakao.maps.LatLng(resultCoords[0].y, resultCoords[0].x);
    ...
    searchDetailAddrFromCoords(coords, function (result, status) {
        if (status === kakao.maps.services.Status.OK) {
            ...
            if (result[0].road_address == null || result[0].road_address.building_name== "") {
                ...
                var address_area = '<a onclick="changeMapPosition2(' + resultCoords[0].y + ', ' + resultCoords[0].x +')"><div class="result_card">';
...
function keyword_search_displayresult(text, resultCoords) {
    for (let i = 0; i < resultCoords.length; i++) {

        var coords = new kakao.maps.LatLng(resultCoords[i].y, resultCoords[i].x);
        searchDetailAddrFromCoords(coords, function (result, status) {
            if (status === kakao.maps.services.Status.OK) {
                ...
                if (result[0].road_address == null  || result[0].road_address.building_name== "") {
                    var address_name = result[0].address.address_name;
                    var place_name = resultCoords[i].place_name;

                    var address_area = '<a onclick="changeMapPosition2(' + resultCoords[i].y + ', ' + resultCoords[i].x +')"><div class="result_card" onclick="">';
                    ...
                    var address_area = '<a onclick="changeMapPosition2(' + resultCoords[i].y + ', ' + resultCoords[i].x +')"><div class="result_card">';
...
//지정한 좌표로 맵 이동
function changeMapPosition(y, x) {
    console.log('맵 이동 하기')
    map.setCenter(new kakao.maps.LatLng(y, x));
}

//지정한 좌표로 맵 이동
function changeMapPosition2(y, x) {
    console.log('맵 이동 하기');
